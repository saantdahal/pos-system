...
class Command(BaseCommand):
    help = 'Add sample data for BCA Chaya Wala restaurant'

    def handle(self, *args, **options):
        # Get the restaurant
        r = Restaurant.objects.filter(slug='bca-chaya-wala').first()
        if not r:
            self.stdout.write(self.style.ERROR('Restaurant bca-chaya-wala not found'))
            return

        self.stdout.write(f'Found restaurant: {r.name}')
        ...
        self.stdout.write(f'Categories: {r.categories.count()}')
        self.stdout.write(f'Menu items: {r.menu_items.count()}')
        self.stdout.write(f'Testimonials: {r.testimonials.count()}')
        self.stdout.write(f'Social links: {SocialMediaLink.objects.filter(landing_page__isnull=True, is_active=True).count()}')
